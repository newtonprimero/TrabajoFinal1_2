...
public class DaoServicio {
    ...
    public List<Servicio> listaFiltros(String i, String f, String id) {
        List<Servicio> lst = new ArrayList<Servicio>();
        String sql = "SELECT taller_utp1.servicio.total, taller_utp1.cita.fecha_hora, "
                + "taller_utp1.aparato.descripcion, taller_utp1.marca.descripcion, "
                + "taller_utp1.persona.nombres, taller_utp1.persona.apellidos FROM "
                + "taller_utp1.servicio INNER JOIN taller_utp1.cita ON "
                + "taller_utp1.servicio.id_cita = taller_utp1.cita.id_cita "
                + "INNER JOIN taller_utp1.aparato ON taller_utp1.cita.id_aparato = taller_utp1.aparato.id_aparato "
                + "INNER JOIN taller_utp1.marca ON taller_utp1.aparato.id_marca = taller_utp1.marca.id_marca INNER "
                + "JOIN taller_utp1.persona ON taller_utp1.servicio.id_tecnico = taller_utp1.persona.id_persona "
                + "WHERE cita.id_persona=" + id + " AND cita.fecha_hora BETWEEN '" + i + "%' AND '" + f + "%';";
        try {
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                Servicio serv = new Servicio();
                serv.setTotal(rs.getInt(1));
                Cita cita = new Cita();
                cita.setFecha_hora(rs.getString(2));
                Aparato aparato = new Aparato();
                aparato.setDescAparato(rs.getString(3));
                Marca marc = new Marca();
                marc.setDescripcion(rs.getString(4));
                Tecnicos persona = new Tecnicos();
                persona.setNombres(rs.getString(5));
                persona.setApellidos(rs.getString(6));
                aparato.setMarca(marc);
                cita.setAparato(aparato);
                serv.setTenico(persona);
                serv.setCita(cita);
                lst.add(serv);
                System.out.println("el servicio tiene " + serv.getTotal());
                System.out.println("");
            }
    ...
    public List<Servicio> listaFiltrosTecnico(String i, String f, String id) {
        List<Servicio> lst = new ArrayList<Servicio>();
        String sql = "SELECT"
                ...
                + " WHERE"
                + " servicio.id_tecnico = " + id + " AND cita.fecha_hora BETWEEN '" + i + "%' AND '" + f + "%'";
        try {
            con = cn.Conexion();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
